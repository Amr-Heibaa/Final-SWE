   ...
   public function __construct()
{
    ...
    /**
     * Update order phase only.
     */
    public function updatePhase(Request $request, Order $order)
    {
        $validated = $request->validate([
            'phase' => [
                'required',
                'in:' . implode(',', OrderPhaseEnum::forDropdown($order->requires_printing))
            ]
        ]);

        $order->update(['current_phase' => $validated['phase']]);

        // If marked as completed, set completed_at
        if ($validated['phase'] === OrderPhaseEnum::COMPLETED->value && !$order->completed_at) {
            $order->update(['completed_at' => now()]);
        }

        return back()->with('success', 'Order phase updated to: ' . OrderPhaseEnum::from($validated['phase'])->label());
    }
    ...
    public function byCustomer(User $customer)
    {
        ...
        return view('orders.by-customer', compact('orders', 'customer'));
    }

    /**
     * Get orders by meeting.
     */
    public function byMeeting(Meeting $meeting)
    {
        ...
        return view('orders.by-meeting', compact('orders', 'meeting'));
    }
    ...
    public function statistics()
    {
        $stats = [
            ...
            'pending_orders' => Order::where('current_phase', OrderPhaseEnum::PENDING->value)->count(),
            'in_progress_orders' => Order::whereNotIn('current_phase', [
                OrderPhaseEnum::PENDING->value,
                OrderPhaseEnum::COMPLETED->value,
                OrderPhaseEnum::CANCELLED->value
            ])->count(),
            'completed_orders' => Order::where('current_phase', OrderPhaseEnum::COMPLETED->value)->count(),
            'cancelled_orders' => Order::where('current_phase', OrderPhaseEnum::CANCELLED->value)->count(),
            'total_revenue' => Order::where('current_phase', OrderPhaseEnum::COMPLETED->value)->sum('total_price'),
            'average_order_value' => Order::where('current_phase', OrderPhaseEnum::COMPLETED->value)->avg('total_price') ?? 0,
        ];

        // Orders by phase
        $ordersByPhase = Order::select('current_phase', DB::raw('COUNT(*) as count'))
            ->groupBy('current_phase')
            ->get()
            ->mapWithKeys(function ($item) {
                return [$item->current_phase => $item->count];
            });
        ...
        return view('orders.statistics', compact('stats', 'ordersByPhase', 'recentOrders'));
    }
    /**
